...
export default function ArticleSearch({ articles, locale, basePath }: Props) {
  ...
  const searcher = new FuzzySearch(articles, ['title', 'description'], { caseSensitive: false });
          ...
          results.map((article, i) => (
            ...
            >
              <div className="flex-1 min-w-0">
                <p className="text-sm font-bold text-white group-hover:opacity-80 transition-opacity leading-snug" style={{ letterSpacing: '-0.01em' }}>
                  {article.title}
                </p>
                {article.description && (
                ...
                <div className="flex gap-2 items-center mt-2 flex-wrap">
                  <span className="text-xs" style={{ color: 'rgba(138,168,152,0.5)', fontVariantNumeric: 'tabular-nums' }}>
                  ...
                  {article.categories?.map((cat, idx) => (
                    <span
                      ...
                      style={{ color: T.green, background: 'rgba(0,106,82,0.1)', letterSpacing: '0.1em' }}
                    >
                      {cat.name}
                    </span>
                  ))}
              ...
              <span className="text-sm font-bold shrink-0 transition-transform group-hover:translate-x-1" style={{ color: T.green }}>
                →
              </span>
            </a>
          ))
