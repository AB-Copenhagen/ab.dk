...
| `/wp-content/uploads/*` (inline post images, PDFs)           | _unresolved — see below_ |                                                                                                                                                                      |
...
3. Accept broken inline images on any external page/search result that hotlinks the old
   URL directly (most inbound links target the _page_, not the image, so this is lower-risk
...
2. Decide the two "⚠" page-vs-post cases in section 1 (match-report content stored as a WP
   _page_ rather than a _post_ — confirm whether it exists as a Strapi article at all).
