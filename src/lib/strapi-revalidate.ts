import { CacheManager, FileCacheDriver, createWebhookHandler, revalidateConfigSchema } from '@datum-cloud/strapi-revalidate';

const STRAPI_URL = import.meta.env.STRAPI_URL ?? 'http://localhost:1337';
const STRAPI_TOKEN = import.meta.env.STRAPI_API_TOKEN as string | undefined;
const STRAPI_WEBHOOK_SECRET = import.meta.env.STRAPI_WEBHOOK_SECRET as string | undefined;
...
const config = revalidateConfigSchema.parse({
  url: STRAPI_URL,
  token: STRAPI_TOKEN,
  cache: { driver: 'file', dir: CACHE_DIR, fallbackDir: `${CACHE_DIR}-fallback` },
